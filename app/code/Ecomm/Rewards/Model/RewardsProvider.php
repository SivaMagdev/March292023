...
class RewardsProvider extends AbstractDataProvider
{
    /**
     * @var ResourceModel\Rewards\Collection
     */
    ...
    public function getData()
    {
        ...
        foreach ($items as $page) {
            ...
            if (isset($_data['rewards_image'])) {
                $image = [];
                $image[0]['name'] = $page->getRewardsImage();
                $image[0]['url'] = $this->storeManager
                ->getStore()
                ->getBaseUrl(
                    \Magento\Framework\UrlInterface::URL_TYPE_MEDIA
                ).'rewards/tmp/rewardsimg/'.$page->getRewardsImage();
                $_data['rewards_image'] = $image;
            }
            $page->setData($_data);
