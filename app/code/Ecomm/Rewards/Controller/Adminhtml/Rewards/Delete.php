    ...
    public function execute()
    {
        ...
        if ($dataId) {
            try {
                ...
                $resultRedirect->setPath('rewardsadmin/rewards/index');
                return $resultRedirect;
            } catch (NoSuchEntityException $e) {
                $this->messageManager->addErrorMessage(__('The data no longer exists.'));
                return $resultRedirect->setPath('rewardsadmin/rewards/index');
            } catch (LocalizedException $e) {
                $this->messageManager->addErrorMessage($e->getMessage());
                return $resultRedirect->setPath('rewardsadmin/rewards/index', ['id' => $dataId]);
            } catch (\Exception $e) {
                $this->messageManager->addErrorMessage(__('There was a problem deleting the data'));
                return $resultRedirect->setPath('rewardsadmin/rewards/edit', ['id' => $dataId]);
            }
        }
