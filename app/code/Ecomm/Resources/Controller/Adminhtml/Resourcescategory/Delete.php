...
class Delete extends Resourcescategory
{
    /**
    ...
    public function execute()
    {
        ...
        if ($dataId) {
            try {
                ...
                $resultRedirect->setPath('resourcesadmin/resourcescategory/index');
                return $resultRedirect;
            } catch (NoSuchEntityException $e) {
                $this->messageManager->addErrorMessage(__('The data no longer exists.'));
                return $resultRedirect->setPath('resourcesadmin/resourcescategory/index');
            } catch (LocalizedException $e) {
                $this->messageManager->addErrorMessage($e->getMessage());
                return $resultRedirect->setPath('resourcesadmin/resourcescategory/index', ['id' => $dataId]);
            } catch (\Exception $e) {
                $this->messageManager->addErrorMessage(__('There was a problem deleting the data'));
                return $resultRedirect->setPath('resourcesadmin/resourcescategory/edit', ['id' => $dataId]);
            }
        }
